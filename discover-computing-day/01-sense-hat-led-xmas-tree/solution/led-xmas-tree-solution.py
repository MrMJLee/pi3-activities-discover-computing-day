...
Tasks:
(1) Decorate the Christmas tree by changing the colours of a few pixels on the tree - the top two pixels must be coloured white;
(2) Modify the program to make the decorations randomly change their colours every half a second -- to simulate Christmas tree lights.
'''
...
# Clours we use to decorate Xmas tree
O = [0, 0, 0]       # no colour
b = [139,69,19]     # brown
d = [0,0,205]       # 
g = [34,139,34]     # green
...
'''
NB: You may ignore this function, which has been commented out
...
def main():
    ...
    # Task 1 solution: Draw the decorated tree - decorations are static
    drawTree(decorated_tree)
    time.sleep(2)

    # Task 2 solution: Make the decorations to change randomly -- twinkling 20 times 
    count = 0
    while (count < 20):
        twinkleStar()
        addTreeDecos()
    ...
    # Finally draw the static decorated tree 
    drawTree(decorated_tree)
    time.sleep(1)
